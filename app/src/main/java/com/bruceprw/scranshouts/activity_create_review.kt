...
class activity_create_review : AppCompatActivity(){
    ...
    private fun getEstablishmentNamesForSpinner(spinner: Spinner) : MutableList<String> = runBlocking{
        ...
        estRoot.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                selectAdapter = ArrayAdapter<String>(this@activity_create_review, android.R.layout.simple_spinner_dropdown_item, restNameList)
    ...
    private fun populateReview(revTitle : String, restName: String, bodyText: String, rating: Int, view: View) {
        ...
        val thisRid = UUID.randomUUID().toString()
    ...
    private fun addReviewToUser(review: Review, user: String) {
       val pushRef = userRoot.child(user).child("reviewsList").child(review.rID!!).setValue(review).addOnSuccessListener(object : OnSuccessListener<Any> {
            override fun onSuccess(p0: Any?) {
                var intent = Intent(this@activity_create_review, activity_main_feed::class.java)
